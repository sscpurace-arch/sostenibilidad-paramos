...
/**
 * Fecha legible en español para los PDF. Un solo sitio para los tres
 * archivos: antes cada uno parseaba a su manera y dos de ellos caían a
 * `new Date()` cuando faltaba la fecha — el PDF decía "Fecha de la
 * evaluación: hoy", que era mentira.
 *
 * Acepta lo que la app produce o recibe:
 *   - Date
 *   - ISO completo ('2026-09-14T17:00:00.000Z')
 *   - Postgres con espacio ('2024-06-15 12:00:00+00', así viene en el seed;
 *     Safari/iPhone no lo parsea sin normalizar)
 *   - solo fecha ('2026-09-14'): se ancla a mediodía para que en Colombia
 *     (UTC-5) no caiga al día anterior
 * Sin fecha o inválida devuelve `fallback`, nunca la fecha de hoy.
 */
export function fechaLegible(valor, { fallback = 'Sin fecha registrada', mes = 'long' } = {}) {
  ...
  if (valor instanceof Date) {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(txt)) {
      ...
      // '2024-06-15 12:00:00+00' → '2024-06-15T12:00:00+00:00'
      txt = txt.replace(' ', 'T').replace(/([+-]\d{2})$/, '$1:00');
    }
    d = new Date(txt);
  }
  if (isNaN(d.getTime())) return fallback;
  return d.toLocaleDateString('es-CO', { year: 'numeric', month: mes, day: 'numeric' });
}

/**
 * Quién hizo la visita, para el encabezado y la firma de los PDF.
 *
 * Prioridad: el snapshot guardado en la evaluación al enviarla
 * (tecnico_nombre / tecnico_cargo) — es quien estuvo ese día, aunque hoy
 * tenga otro cargo. Si la evaluación es anterior a ese snapshot, el perfil
 * actual del usuario. Devuelve null si no hay nada que decir.
 */
export function datosTecnico(evaluacion, tecnico) {
  const nombre = evaluacion?.tecnico_nombre || tecnico?.nombre || null;
  const cargo = evaluacion?.tecnico_cargo || tecnico?.cargo || null;
  if (!nombre) return null;
  return { nombre, cargo };
}

export function lineaElaboradoPor(evaluacion, tecnico) {
  const t = datosTecnico(evaluacion, tecnico);
  if (!t) return null;
  return `Elaborado por: ${[t.nombre, t.cargo, 'PNN Puracé'].filter(Boolean).join(' — ')}`;
}

/** Ubicación del predio para el encabezado: predio · vereda · municipio · hectáreas */
export function lineaUbicacion(productor) {
  const partes = [productor?.nombre_predio, productor?.vereda, productor?.municipio].filter(Boolean);
  const ha = [];
  if (productor?.area_total_ha != null && productor.area_total_ha !== '') ha.push(`${formatoHa(productor.area_total_ha)} ha en total`);
  if (productor?.area_ganaderia_ha != null && productor.area_ganaderia_ha !== '') ha.push(`${formatoHa(productor.area_ganaderia_ha)} ha en ganadería`);
  if (ha.length) partes.push(ha.join(', '));
  return partes.join(' · ');
}
...
export function piePagina(doc) {
  ...
  for (let i = 1; i <= totalPaginas; i++) {
    ...
    // Fecha de impresión, distinta de la fecha de la visita (que va en el
    // encabezado del documento). Se dice explícito para que nadie las confunda.
    doc.text(
      `Documento generado el ${fechaLegible(new Date())} · la fecha de la visita es la del encabezado — Estrategia SSC · PNN Puracé`,
      MARGEN, alto - 8
    );
