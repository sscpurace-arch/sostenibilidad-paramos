...
// esPrueba: los planes de una calificación de prueba se marcan para que
// saveRecord los deje solo en el celular (la evaluación tampoco sube).
export function usePlanAccion(evaluacionId, { esPrueba = false } = {}) {
  ...
  // Espejo en ref: guardarPlan puede ejecutarse desde timers con closure viejo;
  // sin esto, un segundo guardado del mismo indicador generaría un uuid nuevo (fila duplicada).
  ...
  const guardarPlan = useCallback(async (indicadorId, planData) => {
    ...
    const id = planesRef.current[key]?.id || crypto.randomUUID();
    const record = {
      id,
      evaluacion_id: evaluacionId,
      indicador_id: key,
      idea: planData.idea || '',
      especifico: planData.especifico || '',
      medible: planData.medible || '',
      alcanzable: planData.alcanzable || '',
      relevante: planData.relevante || '',
      // `plazo` es DATE en Postgres: '' revienta con 22P02 y el plan nunca sube.
      // Sin fecha va null. (Los 7 planes atascados del 31-jul-2026 eran esto.)
  ...
  const eliminarPlan = useCallback(async (indicadorId) => {
    const key = Number(indicadorId);
    const existing = planesRef.current[key];
    setPlanes(prev => {
      const next = { ...prev };
      delete next[key];
      return next;
