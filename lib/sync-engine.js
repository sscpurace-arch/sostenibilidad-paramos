          ...
          if (!error) {
            ...
            for (const item of realItems) {
              ...
              if (!errFila) {
                ...
                _lastError = `${tabla}: ${errFila.message || errFila}`;
                await db.sync_queue.update(item.localId, {
                  error_count: (item.error_count || 0) + 1,
                  ultimo_error: String(errFila.message || errFila).slice(0, 300),
                });
              }
            }

            // Cerrar también los duplicados del mismo id que se dedujeron
            // arriba: si el id quedó guardado, todas sus entradas sobran.
