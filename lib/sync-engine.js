...
const SEED_VERSION = '2026-08-25-formulas-fotos';
  ...
  try {
    ...
    while (hasMore && navigator.onLine) {
      ...
      const ORDEN = ['productores', 'evaluaciones', 'respuestas_indicadores',
                     'planes_accion', 'diagnosticos', 'fotos_evidencia'];
          ...
          if (!error) {
            ...
            for (const item of realItems) {
              ...
              if (!errFila) {
                ...
                const msg = String(errFila.message || errFila).slice(0, 300);
                errorPorId[item.payload.id] = msg;
                _lastError = `${tabla}: ${msg}`;
              }
            }

            // TODAS las entradas del grupo se resuelven aquí, sin excepción:
            // o quedan sincronizadas, o se les sube el contador de reintentos.
            // Si alguna quedara pendiente con el contador intacto, el `while`
            // de arriba la vuelve a tomar en la siguiente vuelta y el bucle no
            // termina nunca — la app se queda girando y la pantalla parpadea.
            for (const i of ops.upserts) {
              if (idsOk.has(i.payload.id)) {
                await db.sync_queue.update(i.localId, { sync_pending: 0, ultimo_error: null });
              } else {
                await db.sync_queue.update(i.localId, {
                  error_count: (i.error_count || 0) + 1,
                  ultimo_error: errorPorId[i.payload.id] || _lastError,
                });
              }
            }
          }
