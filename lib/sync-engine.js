...
/**
 * Casos en que el cambio ya no tiene sentido y se puede dar por resuelto sin
 * perder nada que el servidor pudiera recibir:
 *  - 23505 en respuestas_indicadores: el servidor ya tiene la respuesta de ese
 *    indicador para esa evaluación (fila duplicada de un bug viejo de ids).
 *  - 23503 cuyo padre tampoco existe en este celular: huérfano de una
 *    evaluación o productor que ya se borró. Si el padre SÍ está en el
 *    celular, se conserva: el que falla es el padre y hay que ver su motivo.
 */
async function seResuelveSolo(item, desc) {
  if (!desc.permanente || item.operacion === 'DELETE') return false;
  const p = item.payload || {};
  if (desc.codigo === '23505' && item.tabla === 'respuestas_indicadores') return true;
  if (desc.codigo === '23503') {
    try {
      if (p.evaluacion_id) return !(await db.evaluaciones.get(p.evaluacion_id));
      if (item.tabla === 'evaluaciones' && p.finca_id) return !(await db.productores.get(p.finca_id));
    } catch (e) { return false; }
  }
  return false;
}

// Marca una entrada de la cola según el error: descartada (resuelta sola),
// fallo permanente (sin más reintentos automáticos) o reintento normal.
async function marcarFallo(item, desc) {
  if (await seResuelveSolo(item, desc)) {
    await db.sync_queue.update(item.localId, {
      sync_pending: 0,
      resuelto: desc.codigo,
      ultimo_error: `Descartado: ${desc.texto}`,
    });
    return;
  }
  if (desc.permanente) {
