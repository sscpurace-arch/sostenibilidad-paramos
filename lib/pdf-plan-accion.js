...
import { crearCursor, encabezado, piePagina, fechaLegible, PNN_VERDE, GRIS_CLARO, dimensionesPngDataUrl } from './pdf-utils';
...
async function dibujarCuadroResumen(doc, { conPlan, planes, detalles, productor, evaluacion }) {
  ...
  doc.text(`Fecha de la evaluación: ${fechaLegible(evaluacion?.fecha)}`, M, y);
  y += 7;
...
export async function descargarPlanAccionPdf({ planes, indicadores, detalles, productor, evaluacion }) {
  ...
  if (conPlan.length > 0) {
    await dibujarCuadroResumen(doc, { conPlan, planes, detalles, productor, evaluacion });
