...
export default function EnProcesoPage() {
  ...
  async function cargarPendientes() {
    ...
    const results = await Promise.all(cals.map(async (c) => {
      const prod = await db.productores.get(c.productor_id);
      const detCount = await db.calificacion_detalle.where('calificacion_id').equals(c.id).count();
      return { ...c, productor: prod, respondidos: detCount };
    }));
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <button 
              ...
              className="bg-[#2E7D32] text-white px-8 py-3 rounded-xl font-bold shadow-md"
            >
              Iniciar Evaluación
            </button>
          </div>
