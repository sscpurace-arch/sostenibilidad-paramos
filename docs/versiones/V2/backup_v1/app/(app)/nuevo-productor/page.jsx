  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center gap-6">
        <div className="w-20 h-20 bg-green-100 text-[#2E7D32] rounded-full flex items-center justify-center text-4xl">✓</div>
        <h2 className="text-2xl font-bold">Productor Registrado</h2>
        <div className="flex flex-col gap-2 w-full">
          <button 
            onClick={() => router.push(`/calificacion/nueva/${form.id || ''}`)}
            className="bg-[#2E7D32] text-white py-4 rounded-xl font-bold"
          >
            Evaluar ahora
          </button>
          <button 
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-5">
        <div>
          ...
          <input 
            type="number" 
            placeholder="Cédula *" 
            className="w-full py-3 border-b border-gray-100 focus:outline-none focus:border-[#2E7D32]"
            value={form.cedula}
        ...
        <div>
          ...
          <div className="mt-4">
            ...
            <select 
              className="w-full py-3 border-b border-gray-100 focus:outline-none bg-white text-lg font-bold text-[#2E7D32]"
        ...
        <button 
          onClick={handleSave}
          disabled={loading}
          className="bg-[#2E7D32] text-white py-4 rounded-xl font-bold shadow-lg disabled:opacity-50 mt-4"
        >
