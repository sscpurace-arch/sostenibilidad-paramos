...
export default function Dashboard() {
  ...
  async function cargarDatos() {
    ...
    const [total, pendientes] = await Promise.all([
      ...
      db.calificaciones.where('estado').equals('en_proceso').count(),
    ]);
  ...
  return (
    <div className="flex flex-col gap-6 pb-10">
      ...
      <section className="bg-[#2E7D32] text-white p-6 rounded-3xl shadow-xl shadow-green-100 relative overflow-hidden">
      ...
      <div className="flex flex-col gap-3">
        <button 
          ...
          className="w-full bg-white text-[#2E7D32] py-5 rounded-2xl font-bold text-lg shadow-sm border border-gray-100 flex items-center px-6 gap-4 active:scale-[0.98] transition-transform text-left"
        >
