      ...
      >
        ...
        {validProducers.map((p) => (
          <Marker 
            ...
            icon={getMarkerIcon(p.color || '#2E7D32')}
          >
            <Popup className="custom-popup">
              <div className="p-1 min-w-[150px]">
                ...
                <button 
                  ...
                  className="w-full bg-[#2E7D32] text-white py-2 rounded-lg text-xs font-bold shadow-sm active:scale-95 transition-all"
                >
                  Ver Perfil
                </button>
              </div>
          ...
          {Array.from(new Set(validProducers.map(p => p.vereda))).sort().map(vereda => {
            ...
            return (
              <div key={vereda || 'none'} className="flex items-center gap-2">
                <span className="w-3.5 h-3.5 rounded-full border border-white shadow-sm" style={{ backgroundColor: prod?.color || '#2E7D32' }}></span>
                <span className="text-[11px] text-gray-700 font-bold leading-tight">{vereda || 'Sin registrar'}</span>
              </div>
            );
          })}
          <div className="flex items-center gap-2 pt-1 border-t mt-1">
            <span className="w-3.5 h-1 border-t-2 border-dashed border-[#2E7D32]"></span>
