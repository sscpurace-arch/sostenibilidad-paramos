...
4. Registros con `es_prueba: true` **nunca** se encolan hacia Supabase — quedan solo en IndexedDB. **Las hijas de una evaluación de prueba (respuestas, planes) deben llevar la marca también** (`payloadRespuesta` y `usePlanAccion` la agregan solo cuando aplica; la columna no existe en Postgres): sin ella la evaluación no subía pero sus respuestas sí se encolaban y el servidor las rechazaba por RLS para siempre (los "41 cambios pendientes" del iPhone, sep-2026).
5. **La cola es del celular, no del usuario.** Si en el mismo teléfono entra otra cuenta, hereda la cola: un técnico no puede subir lo que creó el admin (RLS). `marcarFallo` lo detecta (`contextoPadre` → 'otro-usuario') y lo explica en el banner; `_limpiarColaHuerfana()` descarta al abrir las hijas de evaluaciones de prueba o ya borradas. Errores permanentes (duplicado 23505, columna desconocida 42703/PGRST204…) se marcan de una vez y no se rehabilitan solos; RLS/FK en hijas NO es permanente si el padre local es del mismo usuario (puede no haber subido aún).
